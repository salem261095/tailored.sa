      ...
      >
        ...
        <div className="mt-14 border-t border-black/8">
          {servicesContent.items.map((item, index) => (
            <motion.article
              key={item.number}
              className="group border-b border-black/8 py-8 md:py-10"
              variants={{
            ...
            >
              <div className="grid gap-5 md:grid-cols-[auto_minmax(0,1fr)] md:gap-8 lg:grid-cols-[minmax(5.5rem,0.16fr)_minmax(0,0.52fr)_minmax(0,0.32fr)] lg:items-start">
                <motion.div
                  ...
                  whileHover={prefersReducedMotion ? undefined : { color: "rgb(17 17 17 / 0.62)" }}
                  transition={revealTransition}
                >
                ...
                <div>
                  <motion.p
                    className="max-w-xl text-base leading-8 text-muted md:text-lg md:translate-y-2 md:opacity-0 md:transition-[opacity,transform] md:duration-500 md:[transition-timing-function:cubic-bezier(0.32,0.72,0,1)] md:group-hover:translate-y-0 md:group-hover:opacity-100"
                  >
                    {item.description}
                  </motion.p>
                </div>
